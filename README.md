...
## 이미지 100만개를 한번에 리사이즈하고 용량압축 하기
 이미지 크기를 줄여야 할 때 

파이썬으로 웹서버 만들 때 이미지 압축이 필요하다

이미지 조작을 위해 오픈부터 하자

Image.open(경로)

이미지 리사이즈하는 법 → .resize((가로, 세로)) → 변수에 할당하기

변형한 이미지 저장법 .save(경로와 파일명)
...
img.save('경로와 파일명, quality = 숫자)

PNG파일 저장시 quantize 옵션 찾아보기

PNG → JPG 변환도 쉬움

progressive=True → Progressive JPG 파일 만드는 법

이미지 자르기 .crop((왼쪽 위 좌표, 오른쪽 아래 좌표))

흑백변환 → .convert('L')

images 폴더 내의 많은 이미지를 한번에 리사이즈?
